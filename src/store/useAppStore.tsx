...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const addWarehouse = (newWarehouse: Warehouse) => {
    ...
    addCustomAuditLog('SYSTEM', currentUser?.name || 'Admin', `Added New Warehouse: ${newWarehouse.name} (${newWarehouse.location})`);
  };
  ...
  const updateInventoryStock = (warehouseId: string, productId: string, newStock: number) => {
    setInventory((prev) => {
      ...
      if (exists) {
        return prev.map((inv) =>
          inv.warehouseId === warehouseId && inv.productId === productId
            ? { ...inv, quantityOnHand: Math.max(0, newStock) }
            : inv
        );
      } else {
        return [
    ...
    addCustomAuditLog(
      ...
      `Restocked Inventory: ${targetProd?.name || productId} at ${targetWh?.name || warehouseId} updated to ${newStock} units.`
    );
  };
  ...
  const createNewQuote = (companyId: string): string => {
    ...
    const newQuote: Quote = {
      ...
      fulfillmentBlockReason: 'Quotation is in Draft state.',
      revisionHistory: [
        {
          version: 1,
          updatedBy: currentUser?.name || 'P. Mehta',
          timestamp: new Date().toISOString(),
          promisedDeliveryDate: new Date(Date.now() + 86400000 * 30).toISOString().split('T')[0],
          discountSummary: 'Initial quotation initialized in Draft state',
          status: 'Draft',
        },
      ],
    };
      ...
      prev.map((q) => {
        ...
        if (evalResult.riskLevel === 'HIGH' || evalResult.riskLevel === 'MEDIUM') {
          ...
          addCustomAuditLog(
            quoteId,
            'System Governance',
            'Compliant Deal Auto-Approved & Sent directly to Customer Portal.'
          );
        }
      ...
      prev.map((q) => {
        ...
        addCustomAuditLog(
          ...
          'Submitted Revision Request & Counter Terms via Portal',
          {
            notes,
            requestedDelivery,
            requestedDiscounts: lineDiscounts,
          }
        );
    ...
    setQuotes((prev) =>
      prev.map((q) => {
        ...
        addCustomAuditLog(
          ...
          'Accepted Customer Revision Request',
          {
