...
import { ShieldCheck, Lock, Mail, ArrowRight, ArrowLeft, KeyRound, AlertCircle, UserPlus, Building, User } from 'lucide-react';
...
export const LoginView: React.FC<LoginViewProps> = ({ onBackToLanding }) => {
  const { login, loginAsCustomer, users, addUser, addCompany, setActiveView } = useAppStore();
    ...
    <div className="min-h-screen bg-[#F4F6F9] flex flex-col items-center justify-center p-4">
      <div className="max-w-md w-full mb-3 flex items-center justify-between">
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-[#0176D3] transition cursor-pointer px-2.5 py-1.5 rounded-lg hover:bg-white/80"
        >
          <ArrowLeft className="w-4 h-4" /> Back to Product Landing Page
        </button>
      </div>
      <div className="max-w-md w-full bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden card-3d">
        {/* Header Branding */}
        <div className="bg-[#0176D3] p-6 text-center text-white flex flex-col items-center justify-center space-y-2">
          <BrandLogo size="lg" theme="dark" subtitle="Identity & Access Management" />
          <p className="text-xs text-blue-100 font-medium">
            Entry Point for Internal Stakeholders & Enterprise Customers
          </p>
        </div>
