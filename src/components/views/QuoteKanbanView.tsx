...
import { Plus, Search, LayoutList, Kanban, ChevronRight, Filter, Trash2, X, Building, Check } from 'lucide-react';
...
export const QuoteKanbanView: React.FC<QuoteKanbanViewProps> = ({ forcedMode }) => {
  const { quotes, setSelectedQuoteId, setActiveView, createNewQuote, companies, userRole, currentUser, customerAssignments, deleteQuote } = useAppStore();
  ...
  return (
    <div className="space-y-4 font-sans">
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          {userRole === 'admin' && q.status === 'Draft' && (
                            <button
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                if (confirm(`Are you sure you want to delete draft quotation ${q.id} (${q.companyName})?`)) {
                                  deleteQuote(q.id);
                                }
                              }}
                              className="px-2 py-1 rounded bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold border border-rose-200 transition cursor-pointer text-[11px] flex items-center gap-1 shadow-2xs"
                        ...
                        <div className="flex items-center gap-1.5">
                          {userRole === 'admin' && q.status === 'Draft' && (
                            <button
                              ...
                              className="p-1 rounded bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition cursor-pointer"
                              title="Admin Delete Draft Quote"
                            >
      ...
      {/* MODAL: SELECT CUSTOMER FOR NEW QUOTATION */}
      {isCustomerModalOpen && (
          ...
          <div className="bg-white rounded-xl border border-slate-200 max-w-lg w-full p-6 space-y-4 shadow-xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200">
              <div>
                <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  <Building className="w-5 h-5 text-[#0176D3]" /> Select Customer Account
                </h3>
                <p className="text-xs text-slate-500">
                  Select the enterprise customer account to initialize a new quotation draft.
                </p>
              </div>
              <button
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-lg bg-slate-50 border border-slate-300 text-xs text-slate-900 outline-none focus:border-[#0176D3] focus:bg-white"
              />
            </div>
              ...
              {companies
                .filter((c) => {
                  ...
                  return (
                    ...
                    c.tierId.toLowerCase().includes(query) ||
                    c.contactEmail.toLowerCase().includes(query)
                  );
                })
                .map((comp) => {
                  const assignedRepEmail = customerAssignments[comp.id] || 'rep@dealflow360.com';
                  const isAssignedToCurrentUser = currentUser?.email === assignedRepEmail;

                  return (
                    <div
                      key={comp.id}
                      onClick={() => {
                        const newId = createNewQuote(comp.id);
                        setSelectedQuoteId(newId);
                        setActiveView('builder');
                        setIsCustomerModalOpen(false);
                      }}
                      className={`p-3.5 rounded-lg border transition cursor-pointer flex items-center justify-between group ${
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-xs text-slate-900 group-hover:text-[#0176D3]">
                            {comp.name}
                          </span>
                          ...
                          {isAssignedToCurrentUser && (
                            <span className="px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 text-[10px] font-extrabold flex items-center gap-0.5">
                              <Check className="w-2.5 h-2.5" /> Your Account
                            </span>
                          )}
                        </div>
                        <p className="text-[11px] text-slate-500">
                          {comp.industry} • Credit Limit: ₹{comp.creditLimit.toLocaleString('en-IN')}
                        </p>
                      </div>

                      <button
                        type="button"
                        className="px-3 py-1.5 rounded-md bg-white group-hover:bg-[#0176D3] text-slate-700 group-hover:text-white border border-slate-300 group-hover:border-[#0176D3] text-xs font-bold transition shadow-2xs cursor-pointer"
                      >
                        Create Quote →
                      </button>
                    </div>
            ...
            <div className="pt-2 border-t border-slate-200 flex justify-end">
              <button
                type="button"
                onClick={() => setIsCustomerModalOpen(false)}
                className="px-4 py-2 rounded-lg border border-slate-300 font-bold text-xs text-slate-700 hover:bg-slate-100 cursor-pointer"
              >
                Cancel
              </button>
            </div>
