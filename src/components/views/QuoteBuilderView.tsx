    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <tr key={line.id} className="hover:bg-slate-50/60 transition">
                      <td className="p-3 font-sans">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {line.isRecurring && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-200 shrink-0 shadow-2xs">
                              <Zap className="w-2.5 h-2.5 text-indigo-600 fill-indigo-600" />
                              MRR ({line.billingPeriod || 'Monthly'})
                            </span>
                          )}
                        </div>
                        <span className="text-[11px] text-slate-500 capitalize font-medium block mt-0.5">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 card-3d space-y-4">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-700 block pb-2 border-b border-slate-200">
      ...
      {/* AI Dynamic Engine • Recommendations Section (Sized at 80% screen ratio in a single clean line) */}
      <div className="w-[80%] max-w-7xl mx-auto my-6 bg-white rounded-xl border border-blue-200 p-6 card-3d space-y-4 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
          <div className="flex items-center gap-2.5 text-[#0176D3]">
            <Sparkles className="w-5 h-5 text-purple-600 animate-pulse shrink-0" />
            <span className="text-sm font-black uppercase tracking-wider text-slate-900 whitespace-nowrap">
              AI Deal Dynamic Engine • Recommendations
            </span>
          </div>
            ...
            {upsellRecommendations.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded text-[11px] font-bold transition cursor-pointer flex items-center gap-1.5 ${
                    recommendationFilter === 'cross-sell'
                      ? 'bg-blue-600 text-white shadow-2xs'
                      : 'text-slate-600 hover:bg-slate-100'
                  }`}
                >
                  <ArrowRight className="w-3 h-3 text-blue-600" /> Cross-Sell ({crossSellCount})
                </button>
                <button
                ...
                >
                  <TrendingUp className="w-3 h-3 text-purple-600" /> Upsell ({upsellCount})
                </button>
              </div>
            )}

            {totalPotentialMarginBoost > 0 && (
              <span className="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 shadow-2xs">
                +₹{totalPotentialMarginBoost.toLocaleString('en-IN')} Margin Boost
              </span>
            )}
          </div>
        </div>
        ...
        <p className="text-xs text-slate-500 font-medium">
          {activeQuote.lines.length > 0
            ? `Analyzing ${activeQuote.lines.length} quote item${activeQuote.lines.length > 1 ? 's' : ''} in real-time with AI-driven correlation logic for high-margin attachments.`
        ...
        {filteredRecommendations.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRecommendations.slice(0, 3).map((rec) => (
              <div
                key={rec.productId}
                className="p-4 rounded-xl bg-white border border-slate-200 hover:border-blue-400 space-y-3 card-hover-3d shadow-2xs flex flex-col justify-between"
              >
                <div className="space-y-2.5">
                  ...
                  <p className="text-[11px] text-slate-600 font-medium leading-relaxed bg-slate-50/80 p-2.5 rounded-lg border border-slate-100">
                    {rec.reason}
                  </p>
                </div>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2 mt-2">
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-1 text-[10px] text-emerald-700 font-bold">
                      <span>+{rec.expectedMarginPercent}% margin</span>
                      <span>(+₹{rec.expectedMarginImpact.toLocaleString('en-IN')})</span>
                    </div>
                  </div>
                  ...
                  {canEditQuote && (
                    <button
                      onClick={() => {
                        addLineToQuote(activeQuote.id, rec.productId, 1, rec.promoDiscount || 0);
                        setSaveSuccessToast(true);
                        setTimeout(() => setSaveSuccessToast(false), 2500);
                      }}
